...
async function compareSchemas() {
    ...
    try {
        ...
        console.log('Total tables in prod:', prodTables.length);
        console.log('Total tables in dev:', devTables.length);

        if (missingTablesInDev.length > 0) {
            console.log('\n[MISSING TABLES IN DEV (exist in prod but NOT in dev)]:', JSON.stringify(missingTablesInDev, null, 2));
        } else {
            console.log('\nNo missing tables in dev.');
        }

        if (extraTablesInDev.length > 0) {
            console.log('\n[EXTRA TABLES IN DEV (exist in dev but NOT in prod)]:', JSON.stringify(extraTablesInDev, null, 2));
        } else {
            console.log('\nNo extra tables in dev.');
        }
