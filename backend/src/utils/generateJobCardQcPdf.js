...
<body>
    ...
    const renderData = {
      ...
      reportNo: `JCQC-${new Date(log.check_date || Date.now()).getFullYear()}-${String(log.id).padStart(4, '0')}`,
      check_date: formatDate(log.check_date),
      shift: log.shift === 'SHIFT_A' ? 'A' : log.shift === 'SHIFT_B' ? 'B' : 'C',
      job_card_no: log.job_card_no,
      wo_number: log.wo_number,
      operation_name: log.operation_name,
      project_name: log.project_name || 'Stock/Internal',
      client_name: log.client_name || 'Internal',
      item_name: log.item_name,
      item_code: log.item_code,
      inspected_qty: parseFloat(log.inspected_qty || 0).toFixed(3),
      accepted_qty: parseFloat(log.accepted_qty || 0).toFixed(3),
      rejected_qty: parseFloat(log.rejected_qty || 0).toFixed(3),
      scrap_qty: parseFloat(log.scrap_qty || 0).toFixed(3),
      status: log.status,
      statusClass: log.status === 'APPROVED' ? 'status-approved' : 'status-pending',
