...
const getQCItems = async (qcId) => {
  const [items] = await pool.query(
    `SELECT 
      ...
      qci.created_at,
      qci.updated_at,
      COALESCE(poi.material_name, gi.material_name) as material_name,
      COALESCE(poi.description, gi.remarks) as description,
      COALESCE(gi.length, poi.length, 0) as length,
      COALESCE(gi.width, poi.width, 0) as width,
      COALESCE(gi.thickness, poi.thickness, 0) as thickness,
      COALESCE(gi.diameter, poi.diameter, 0) as diameter,
      COALESCE(gi.outer_diameter, poi.outer_diameter, 0) as outer_diameter,
