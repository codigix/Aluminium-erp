...
async function optimizeAndProfile() {
  const config = {
  ...
  // Format dates with time for indexable comparison
  const startDateTime = `${startRange} 00:00:00`;
  const endDateTime = `${endRange} 23:59:59`;

  let dateFilter = ' AND created_at BETWEEN ? AND ?';
  ...
  const runQuery = async (name, sql, queryParams) => {
    ...
    console.log(`Optimized Query "${name}" took ${duration} ms (returned ${rows.length === undefined ? 1 : rows.length} rows)`);
    return duration;
  };

  try {
    console.log('Adding indexes if they do not exist...');
    
    // Add index to orders
    try {
      await connection.query('CREATE INDEX idx_orders_created_at_status ON orders(created_at, status)');
      console.log('Created index idx_orders_created_at_status on orders table.');
    } catch (e) {
      if (e.code === 'ER_DUP_KEYNAME') {
        console.log('Index idx_orders_created_at_status already exists.');
      } else {
        console.error('Error creating index on orders:', e.message);
      }
    }

    // Add index to customer_pos
    try {
      await connection.query('CREATE INDEX idx_customer_pos_created_at_status ON customer_pos(created_at, status)');
      console.log('Created index idx_customer_pos_created_at_status on customer_pos table.');
    } catch (e) {
      if (e.code === 'ER_DUP_KEYNAME') {
        console.log('Index idx_customer_pos_created_at_status already exists.');
      } else {
        console.error('Error creating index on customer_pos:', e.message);
      }
    }
    ...
    await runQuery('3. Sales Trend', `
      ...
      LEFT JOIN orders o ON o.created_at >= date_list.date AND o.created_at < date_list.date + INTERVAL 1 DAY AND o.status != 'CANCELLED'
