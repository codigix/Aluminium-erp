      ...
      {shouldShowSearchToolbar && (
        <div className="flex flex-col md:flex-row justify-between items-center p-3 gap-4 border-b border-slate-100 bg-white">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-end">
             ...
             <button type="button" className="flex items-center gap-2 px-3 py-2 text-sm font-semibold rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 transition-colors">
                <Bookmark className="w-4 h-4" />
                Saved Views
             </button>

             <div className="hidden md:flex items-center border border-slate-200 rounded-lg p-0.5 bg-slate-50 ml-2">
                <button type="button" onClick={() => setViewMode('list')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
                   <List className="w-4 h-4" />
                </button>
                <button type="button" onClick={() => setViewMode('grid')} className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
         ...
         <div className="flex items-center gap-2 p-3 bg-slate-50/50 border-b border-slate-100 flex-wrap">
            ...
            {activeFilters.map((filter, idx) => (
               <div key={idx} className="flex items-center gap-1.5 px-2.5 py-1 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-md text-xs font-medium">
                  ...
                  <button type="button" onClick={() => onFilterRemove && onFilterRemove(filter.id)} className="ml-1 hover:text-indigo-900 transition-colors p-0.5 rounded-full hover:bg-indigo-200">
                     <X className="w-3 h-3" />
                  </button>
               </div>
            ))}
            <button type="button" onClick={onClearFilters} className="flex items-center gap-1.5 px-2.5 py-1 text-slate-500 hover:text-rose-600 text-xs font-semibold ml-auto transition-colors">
         ...
         <div className="flex items-center gap-3 p-2 bg-indigo-50/50 border-b border-indigo-100 px-4 slide-down animate-in duration-200">
            ...
            {bulkActions.length > 0 ? (
               bulkActions.map((action, idx) => (
                  <button type="button" key={idx} onClick={() => action.onClick(selectedRows)} className={`flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-md transition-colors ${action.className || 'text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-900 shadow-sm'}`}>
               ...
               <>
                  <button type="button" className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-md text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-900 shadow-sm">
                     <Edit2 className="w-4 h-4" /> Edit
                  </button>
                  <button type="button" className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-md text-rose-600 bg-white border border-slate-200 hover:bg-rose-50 hover:text-rose-700 hover:border-rose-200 shadow-sm">
                     <Trash2 className="w-4 h-4" /> Delete
                  </button>
                  <button type="button" className="flex items-center gap-2 px-3 py-1.5 text-sm font-semibold rounded-md text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-900 shadow-sm ml-auto">
          ...
          <div className="flex items-center gap-6">
             ...
             {totalPages > 1 && (
               <div className="flex items-center gap-1">
                 <button type="button" onClick={() => handlePageChange(1)} disabled={currentPage === 1} className="p-1.5 rounded-md hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed text-slate-600 transition-colors">
                   <ChevronsLeft className="h-4 w-4" />
                 </button>
                 <button type="button" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-1.5 rounded-md hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed text-slate-600 transition-colors">
                 ...
                 <button type="button" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-1.5 rounded-md hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed text-slate-600 transition-colors">
                   <ChevronRight className="h-4 w-4" />
                 </button>
                 <button type="button" onClick={() => handlePageChange(totalPages)} disabled={currentPage === totalPages} className="p-1.5 rounded-md hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed text-slate-600 transition-colors">
         ...
         <>
            ...
            <div className="fixed inset-y-0 right-0 w-[350px] bg-white shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-100">
               <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
                  ...
                  <div className="flex items-center gap-2">
                     <button type="button" onClick={onClearFilters} className="text-xs font-semibold text-slate-500 hover:text-slate-700 transition-colors">Reset</button>
                     <button type="button" onClick={() => setShowFilterPanel(false)} className="p-1.5 rounded-md hover:bg-slate-200 text-slate-500 transition-colors">
                        <X className="w-4 h-4" />
                     </button>
                  </div>
               </div>
               ...
               <div className="p-4 border-t border-slate-100 flex items-center gap-3 bg-white">
                  <button type="button" onClick={() => setShowFilterPanel(false)} className="flex-1 py-2 px-4 rounded-lg font-bold text-sm text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 transition-colors">
                     Cancel
                  </button>
                  <button type="button" onClick={() => { onApplyFilters && onApplyFilters(); setShowFilterPanel(false); }} className="flex-1 py-2 px-4 rounded-lg font-bold text-sm text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition-colors">
                     Apply Filters
                  </button>
               </div>
            </div>
