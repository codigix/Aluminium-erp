...
const StarRating = ({ rating }) => {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="text-xs  text-slate-600 ml-1">{numRating.toFixed(1)}</span>
    </div>
  );
};
...
const Vendors = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'category',
      sortable: true,
      render: (val) => val ? <span className="px-2 py-1 bg-blue-50 text-blue-600 rounded  text-xs  border border-blue-100  ">{val}</span> : '—'
    },
    {
      label: 'Contact',
      key: 'contact',
  ...
  const renderVendorExpanded = (vendor) => (
    <div className="bg-slate-50/50 p-2 rounded border border-slate-100 m-2">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-xs  text-slate-900  ">Contact Information</h4>
          </div>
          <div className="bg-white p-2 rounded border border-slate-100 space-y-3">
            <div>
              <p className="text-xs  text-slate-400 ">Email Address</p>
              <p className="text-xs  text-slate-700">{vendor.email || 'N/A'}</p>
            </div>
            <div>
              <p className="text-xs  text-slate-400 ">Phone Number</p>
              <p className="text-xs  text-slate-700">{vendor.phone || 'N/A'}</p>
            </div>
            <div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-xs  text-slate-900  ">Business Summary</h4>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white p-2 rounded border border-slate-100">
              <p className="text-xs  text-slate-400 ">Total Orders</p>
              <p className="text-md  text-slate-900">{vendor.total_orders || 0}</p>
            </div>
            <div className="bg-white p-2 rounded border border-slate-100">
              <p className="text-xs  text-slate-400 ">Total Value</p>
              <p className="text-lg  text-indigo-600">₹{(vendor.total_value || 0).toLocaleString('en-IN')}</p>
            </div>
            <div className="bg-white p-2 rounded border border-slate-100 col-span-2">
              <p className="text-xs  text-slate-400 ">Last Order Date</p>
              <p className="text-xs  text-slate-700 flex items-center gap-1">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-xs  text-slate-900  ">Classification</h4>
          </div>
          <div className="bg-white p-2 rounded border border-slate-100 space-y-3">
            <div>
              <p className="text-xs  text-slate-400 ">Vendor Type</p>
              <p className="text-xs  text-slate-700 ">{vendor.vendor_type || 'N/A'}</p>
            </div>
            <div>
              <p className="text-xs  text-slate-400 ">Category</p>
              <p className="text-xs  text-slate-700 ">{vendor.category || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="p-2 space-y-2 bg-slate-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-slate-500 ml-14">Manage your supplier network and relationships</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div className="p-2 bg-white border border-slate-100 shadow-sm rounded flex items-center gap-2">
          <div className="p-2 rounded bg-indigo-50 text-indigo-600">
            <Users className="w-5 h-5" />
          </div>
          <div>
            <p className="text-xs  text-slate-400   mb-1">Total Suppliers</p>
            <p className="text-xl  text-slate-900">{stats?.totalVendors || vendors.length}</p>
          </div>
        </div>
        <div className="p-2 bg-white border border-slate-100 shadow-sm rounded flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xl  text-slate-900">{vendors.filter(v => v.status === 'ACTIVE').length}</p>
          </div>
        </div>
        <div className="p-2 bg-white border border-slate-100 shadow-sm rounded flex items-center gap-2">
        ...
        <div className="p-2 bg-white border border-slate-100 shadow-sm rounded flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xl  text-slate-900">{vendors.reduce((acc, v) => acc + (v.total_orders || 0), 0)}</p>
          </div>
        </div>
      </div>
      ...
      <Modal
        ...
        size="3xl"
      >
        <form onSubmit={handleAddVendor} className="space-y-2">
          <div className="space-y-2">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              <Users className="w-4 h-4 text-indigo-500" />
              <h3 className="text-sm  text-slate-800  ">Basic Information</h3>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              ...
              <h3 className="text-sm  text-slate-800  ">Contact Information</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <FormControl label="Location/Address">
              <textarea
                ...
                className="w-full p-2 bg-white border border-slate-200 rounded text-xs focus:ring-2 focus:ring-indigo-500 outline-none transition-all min-h-[100px]"
              />
            </FormControl>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              ...
              <h3 className="text-sm  text-slate-800  ">Performance</h3>
            </div>
            <FormControl label="Supplier Rating (0.0 - 5.0)">
              <div className="flex items-center gap-6">
                <input
                  ...
                  className="w-32 p-2 bg-white border border-slate-200 rounded text-xs  focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                />
          ...
          <div className="flex gap-2 justify-end pt-6 border-t border-slate-100">
            <button
              ...
              className="px-6 py-2.5 border border-slate-200 rounded  text-slate-600 text-xs  hover:bg-slate-50 transition-all"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-10 py-2.5 bg-indigo-600 text-white rounded text-xs  hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95"
            >
