  ...
  if (loading || !stats) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <p className="text-xs text-slate-500 font-bold tracking-widest uppercase">Initializing Production Command Center...</p>
      </div>
    );
  }
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">Production Floor</h1>
        ...
        <div className="xl:col-span-2 bg-white rounded-[40px] p-8 border border-slate-100 shadow-sm">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase mt-1 tracking-widest">DAILY THROUGHPUT & EFFICIENCY</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold mt-0.5 uppercase tracking-widest">REAL-TIME PRODUCTION PIPELINE</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/20">
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">WO Code</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Item</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Quantity</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {(stats.priorityOrders || []).map((wo, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-4 text-xs font-black text-slate-900">{wo.wo_code}</td>
                  <td className="px-8 py-4 text-xs font-medium text-slate-600">{wo.item_name}</td>
                  <td className="px-8 py-4 text-xs font-medium text-slate-600">{wo.quantity} {wo.unit}</td>
              ...
              {(!stats.priorityOrders || stats.priorityOrders.length === 0) && (
                <tr>
                  <td colSpan="4" className="px-8 py-12 text-center text-xs text-slate-400 font-bold uppercase tracking-widest">No active work orders found</td>
                </tr>
              )}
            </tbody>
          </table>
