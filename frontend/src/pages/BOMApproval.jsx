    ...
    { 
      ...
      render: (_, item) => (
        <div>
          ...
          <div className="text-xs text-slate-500">Sales Order Ref</div>
        </div>
      )
    },
    { 
        ...
        <div className="flex justify-end gap-2">
          ...
          {activeTab === 'pending' && (
            <button 
              ...
              className="p-1.5 rounded  border border-slate-200 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-all shadow-sm"
              title="Approve"
            >
          ...
          <div className="flex items-center justify-center min-h-screen px-4 py-8">
            ...
            <div className="relative bg-white rounded  shadow-2xl max-w-6xl w-full overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-xs  text-indigo-500  ">{selectedOrder?.project_name}</span>
                    </div>
                  </div>
                </div>
                <button 
                  ...
                  <div className="space-y-2">
                      ...
                      <div className="bg-white p-2 rounded border border-slate-100 shadow-sm flex items-center justify-between group hover:border-indigo-200 transition-colors">
                        <div>
                          ...
                          <div className="flex items-baseline gap-1">
                            <span className="text-xl  text-slate-900 er">{orderItems.filter(i => i.status !== 'REJECTED').length}</span>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {orderItems.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="bg-white border border-slate-100 rounded overflow-hidden shadow-sm hover: transition-all group border-l-4 border-l-transparent hover:border-l-indigo-500">
                            <div 
                              className="p-2 flex flex-col lg:flex-row lg:items-center justify-between gap-2 cursor-pointer"
                              onClick={() => toggleItem(item.id)}
                            >
                              <div className="flex items-center gap-2 min-w-[280px]">
                                <div className={`w-5 h-5 rounded  flex items-center justify-center transition-colors  ${isExpanded ? 'bg-indigo-600 text-white' : 'bg-slate-50 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-500'}`}>
                                  <FileText size={15} />
                                </div>
                                <div>
                                  <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex items-center gap-2 mt-1.5" onClick={e => e.stopPropagation()}>
                                    <button 
                                      ...
                                      className="text-xs  text-indigo-500 hover:text-indigo-700 underline underline-offset-4 decoration-indigo-200   flex items-center gap-1"
                                    >
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 flex-1 max-w-2xl px-2">
                                <div className="p-2 rounded hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                                  <p className="text-[8px]  text-slate-400  tracking-[0.15em] mb-1">Order Qty</p>
                                  <p className="text-[13px]  text-slate-900">{parseFloat(item.quantity || 0).toLocaleString()} <span className="text-xs text-slate-400  ">{item.unit || 'Nos'}</span></p>
                                </div>
                                <div className="p-2 rounded hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                                  <p className="text-[8px]  text-slate-400  tracking-[0.15em] mb-1">Material Cost</p>
                                  <p className="text-[13px]  text-slate-900">₹{(matCost + compCost).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                                </div>
                                <div className="p-2 rounded hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                                  <p className="text-[8px]  text-slate-400  tracking-[0.15em] mb-1">Labor Cost</p>
                                  <p className="text-[13px]  text-slate-900">₹{laborCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
                                </div>
                                <div className="bg-emerald-50/30 p-2 rounded border border-emerald-100/50 flex flex-col justify-center">
                                  <p className="text-[8px]  text-emerald-600  tracking-[0.15em] mb-1">Est. Profit</p>
                                  <p className="text-[13px]  text-emerald-700">₹0.00</p>
                                </div>
                              </div>
                            ...
                            {isExpanded && (
                              <div className="p-2 bg-slate-50/50 border-t border-slate-100 animate-in slide-in-from-top-2 duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                                  {/* Raw Materials */}
                                  <div className="bg-white p-2 rounded border border-slate-100 shadow-sm">
                                    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-50">
                                      <h5 className="text-xs  text-slate-900   flex items-center gap-2">
                                        <div className="w-1.5 h-4 bg-indigo-500 rounded" />
                                        Raw Materials
                                      </h5>
                                      <span className="text-xs  text-indigo-600">₹{matCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                    <div className="space-y-2">
                                      {item.materials?.length > 0 ? item.materials.map((m, idx) => (
                                        <div key={idx} className="flex justify-between items-start gap-2 py-1">
                                          <div className="flex-1">
                                            <p className="text-xs  text-slate-700 leading-tight">{m.material_name}</p>
                                            <p className="text-[8px] text-slate-400  mt-0.5">{m.qty_per_pc} {m.unit} @ ₹{m.rate}</p>
                                          </div>
                                          <span className="text-xs  text-slate-900 whitespace-nowrap">₹{(parseFloat(m.qty_per_pc || 0) * parseFloat(item.quantity || 0) * parseFloat(m.rate || 0)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                        </div>
                                      )) : (
                                        <p className="text-xs text-slate-400 italic">No raw materials listed</p>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white p-2 rounded border border-slate-100 shadow-sm">
                                    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-50">
                                      <h5 className="text-xs  text-slate-900   flex items-center gap-2">
                                        <div className="w-1.5 h-4 bg-blue-500 rounded" />
                                        Components
                                      </h5>
                                      <span className="text-xs  text-blue-600">₹{compCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                    <div className="space-y-2">
                                      {item.components?.length > 0 ? item.components.map((c, idx) => (
                                        <div key={idx} className="flex justify-between items-start gap-2 py-1">
                                          <div className="flex-1">
                                            <p className="text-xs  text-slate-700 leading-tight">{c.component_name}</p>
                                            <p className="text-[8px] text-slate-400  mt-0.5">{c.quantity} {c.unit} @ ₹{c.rate}</p>
                                          </div>
                                          <span className="text-xs  text-slate-900 whitespace-nowrap">₹{(parseFloat(c.quantity || 0) * parseFloat(item.quantity || 0) * parseFloat(c.rate || 0)).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                        </div>
                                      )) : (
                                        <p className="text-xs text-slate-400 italic">No components listed</p>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  <div className="bg-white p-2 rounded border border-slate-100 shadow-sm">
                                    <div className="flex items-center justify-between mb-3 pb-2 border-b border-slate-50">
                                      ...
                                      <span className="text-xs  text-amber-600">₹{laborCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                    <div className="space-y-2">
                                      {item.operations?.length > 0 ? item.operations.map((o, idx) => {
                                        ...
                                        return (
                                          <div key={idx} className="flex justify-between items-start gap-2 py-1">
                                            <div className="flex-1">
                                              <p className="text-xs  text-slate-700 leading-tight">{o.operation_name}</p>
                                              <p className="text-[8px] text-slate-400  mt-0.5">{(parseFloat(o.cycle_time_min || 0) + parseFloat(o.setup_time_min || 0))} MIN @ ₹{o.hourly_rate}/hr</p>
                                            </div>
                                            <span className="text-xs  text-slate-900 whitespace-nowrap">₹{opCost.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                          </div>
                                        );
                                      }) : (
                                        <p className="text-xs text-slate-400 italic">No operations listed</p>
                                      )}
                                    </div>
                                  </div>
                                </div>
              ...
              <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-2">
                <button 
                  onClick={() => setShowDetails(false)}
                  className="px-8 p-2 bg-white border border-slate-200 text-slate-600 rounded text-xs  hover:bg-slate-100 transition-all shadow-sm"
                >
                  Close
                </button>
                {activeTab === 'pending' && (
                  <button 
                    ...
                    className="p-2 bg-indigo-600 text-white rounded text-xs  hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 active:scale-95"
                  >
                    Approve BOM
                  </button>
                )}
