      ...
      <Card title="Final Quality Control" subtitle="Post-production quality clearance and certification">
        <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-2">
          <div className="relative w-full md:w-96">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded  border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              value={searchQuery}
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-slate-50 rounded border border-dashed border-slate-200">
            <CheckCircle className="w-5 h-5 text-emerald-500 mx-auto mb-3 opacity-20" />
                ...
                {filteredOrders.map(order => (
                  <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      ...
                      <p className="text-slate-400 text-xs">{order.project_name || '—'}</p>
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {order.target_dispatch_date ? new Date(order.target_dispatch_date).toLocaleDateString('en-IN') : '—'}
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs font-semibold ${order.production_priority === 'HIGH' ? 'text-rose-600' : 'text-slate-500'}`}>
                      ...
                      <div className="flex justify-end gap-2">
                        {order.status !== 'READY_FOR_SHIPMENT' && (
                          <>
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded  transition-colors bg-white border border-slate-100"
                              title="Reject QC"
                            >
                        ...
                        {order.status === 'READY_FOR_SHIPMENT' && !order.shipment_id && (
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded  transition-colors bg-white border border-slate-100"
                        ...
                        {order.shipment_id && (
                           <span className="inline-flex items-center px-2.5 py-0.5 rounded text-xs bg-indigo-100 text-indigo-800 border border-indigo-200">
