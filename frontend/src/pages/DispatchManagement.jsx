      ...
      case 'READY_TO_DISPATCH':
        return (
          <button 
            ...
            className="p-1.5 bg-indigo-600 text-white text-xs  rounded  hover:bg-indigo-700 transition-all"
          >
            Start Dispatch
          </button>
        );
      case 'DISPATCHED':
        return (
          <button 
            ...
            className="p-1.5 bg-purple-600 text-white text-xs  rounded  hover:bg-purple-700 transition-all"
          >
            In Transit
          </button>
        );
      case 'IN_TRANSIT':
        return (
          <button 
            ...
            className="p-1.5 bg-orange-600 text-white text-xs  rounded  hover:bg-orange-700 transition-all"
          >
            Out for Delivery
          </button>
        );
        ...
        return (
          <div className="flex gap-2">
            <button 
              ...
              className="p-1.5 bg-emerald-600 text-white text-xs  rounded  hover:bg-emerald-700 transition-all"
            >
              Mark Delivered
            </button>
            <button 
              onClick={handleInitiateReturn}
              className="p-1.5 bg-rose-50 text-rose-600 text-xs  rounded  hover:bg-rose-100 transition-all border border-rose-100"
            >
              Return
            </button>
          </div>
        ...
        return (
          <div className="flex gap-2">
            <button 
              ...
              className="p-1.5 bg-slate-700 text-white text-xs  rounded  hover:bg-slate-800 transition-all"
            >
              Close
            </button>
            <button 
  ...
  return (
    <div className="p-2 space-y-2 bg-white/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
        <div>
          <h1 className="text-xl  text-slate-900">Dispatch Management</h1>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-2 bg-white p-8 rounded  border border-slate-100 shadow-sm overflow-x-auto">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-6">
                <div className="relative w-full md:w-96">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 rounded  border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                  />
                </div>
                <div className="flex items-center gap-2 p-1.5 bg-slate-50 border border-slate-100 rounded ">
                   <Truck className="w-4 h-4 text-indigo-500" />
                   <span className="text-xs  text-slate-500  ">Active Shipments: {dispatches.length}</span>
                </div>
              </div>
        ...
        <div className="space-y-2">
          <Card className="bg-white border border-slate-100 rounded  shadow-sm overflow-hidden">
            <div className="p-2 space-y-2">
              <h3 className="text-xs  text-slate-900   border-b border-slate-50 pb-4">Today's Summary</h3>
                ...
                <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs  text-slate-900">{statusCounts.dispatched} Shipments</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs  text-slate-900">{statusCounts.inTransit + statusCounts.outForDelivery} Deliveries</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 bg-slate-50 rounded border border-slate-100">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs  text-slate-900">{statusCounts.delivered} Completed</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-50">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-xl  text-slate-900">1.2 Days</p>
          ...
          <div className="space-y-2 p-2">
              ...
              {['READY_TO_DISPATCH', 'DISPATCHED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY', 'DELIVERED'].map((status, idx) => {
                ...
                return (
                  <div key={status} className="flex flex-col items-center relative">
                    <div className={`w-8 h-8 rounded flex items-center justify-center border-2 z-10 transition-all ${isCompleted ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-300'}`}>
                      {isCompleted ? <CheckCircle className="w-4 h-4" /> : <span>{idx + 1}</span>}
                    </div>
                    <span className={`text-xs  mt-2   ${isCompleted ? 'text-indigo-600' : 'text-slate-400'}`}>
                      {status.replace(/_/g, ' ')}
                    </span>
                    {idx < 4 && (
                      <div className={`absolute top-2 left-[60%] w-full h-[2px] -z-0 ${isCompleted ? 'bg-indigo-600' : 'bg-slate-100'}`} />
                    )}
                  </div>
                );
              })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <div className="p-6 bg-slate-50 rounded  border border-slate-100 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <p className="text-xs  text-slate-900">{selectedItem.so_number || (selectedItem.sales_order_id ? `SO-${String(selectedItem.sales_order_id).padStart(4, '0')}` : '—')}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-400  ">Target Date</p>
                      <p className="text-xs  text-slate-900">{selectedItem.dispatch_target_date ? new Date(selectedItem.dispatch_target_date).toLocaleDateString('en-IN') : '—'}</p>
                    </div>
                  </div>
                  <div className="pt-4 border-t border-slate-200/50">
                    <p className="text-xs text-slate-400   mb-2">Customer Details</p>
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <User className="w-3.5 h-3.5 text-slate-400" />
                        <span className="text-sm  text-slate-700">{selectedItem.customer_name || selectedItem.company_name}</span>
                      </div>
                ...
                {/* Transport Info */}
                <div className="p-6 bg-white rounded  border border-slate-100 shadow-sm space-y-2">
                  <h4 className="text-xs  text-slate-400   flex items-center gap-2">
                    <Truck className="w-4 h-4 text-indigo-500" />
                    Logistics & Transport
                  </h4>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <p className="text-xs text-slate-400   text-xs">Transporter</p>
                      <p className="text-xs  text-slate-900">{selectedItem.transporter || 'Self Managed'}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-slate-400  ">Vehicle Number</p>
                      <div className="px-2 py-1 bg-indigo-50 text-indigo-700 rounded  text-xs  border border-indigo-100 w-fit ">
                        {selectedItem.vehicle_number || 'Pending'}
                      </div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-slate-400  ">Driver Name</p>
                      <p className="text-xs  text-slate-900">{selectedItem.driver_name || 'Unassigned'}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-slate-400  ">Driver Contact</p>
                      <p className="text-xs  text-slate-900">{selectedItem.driver_contact || 'N/A'}</p>
                    </div>
                    <div className="space-y-1 col-span-2">
                      <p className="text-xs text-slate-400  ">Driver Email</p>
                      <p className="text-xs  text-slate-900">{selectedItem.driver_email || 'N/A'}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Items Summary */}
              <div className="space-y-2">
                <div className="bg-white rounded  border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
            ...
            <div className="flex justify-end gap-2 pt-6 border-t border-slate-100">
              <button
                ...
                className="px-8 py-2.5 bg-slate-900 text-white rounded text-xs  hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-200"
              >
                Close Details
              </button>
              {selectedItem.shipment_status === 'READY_TO_DISPATCH' && (
                <button
                  ...
                  className="px-8 py-2.5 bg-indigo-600 text-white rounded text-xs  hover:bg-indigo-700 transition-all active:scale-95 shadow-lg shadow-indigo-200 flex items-center gap-2"
                >
                  <Truck className="w-4 h-4" />
                  Dispatch Now
                </button>
