  ...
  if (loading || !stats) {
    return (
      <div className="flex flex-col items-center justify-center p-22 space-y-2">
        ...
        <p className="text-xs text-slate-500   ">Initializing Sales Performance Hub...</p>
      </div>
    );
  }
    ...
    <div className="space-y-2 pb-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="bg-white rounded] p-2 border border-slate-100 shadow-sm flex flex-col">
          <h3 className="text-md  text-slate-900  mb-2">Pipeline Health</h3>
      ...
      <div className="bg-white rounded  border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-2 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
          <div>
            ...
            <p className="text-xs text-slate-500  mt-0.5  ">TOP PENDING QUOTATIONS & CONTRACTS</p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/20">
                ...
                <th className="px-8 py-4 text-xs  text-slate-400  ">Value</th>
                <th className="px-8 py-4 text-xs  text-slate-400  ">Age</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {(stats.topOpportunities || []).map((opp, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-4 text-xs  text-slate-900">{opp.client_name}</td>
                  <td className="px-8 py-4 text-xs text-slate-600">₹{(opp.value || 0).toLocaleString()}</td>
                  <td className="px-8 py-4 text-xs text-slate-600">{opp.age} days</td>
              ...
              {(!stats.topOpportunities || stats.topOpportunities.length === 0) && (
                <tr>
                  <td colSpan="4" className="px-8 py-12 text-center text-xs text-slate-400   ">No active high-value opportunities</td>
                </tr>
              )}
            </tbody>
          </table>
