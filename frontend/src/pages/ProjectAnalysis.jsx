      ...
      <div className="space-y-2 pb-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        {detailTab === 'Production Flow' && (
          <div className="space-y-2">
             ...
             <div className="grid grid-cols-4 gap-2">
                {[
                  { id: 'ALL', l: 'All Stages', c: productionFlow.length, icon: Layers, bg: 'bg-indigo-600' },
                  { id: 'PENDING', l: 'Pending', c: productionFlow.filter(f => f.status === 'PENDING').length, icon: Clock, bg: 'bg-slate-500' },
                  { id: 'IN_PROGRESS', l: 'In Progress', c: productionFlow.filter(f => f.status === 'IN_PROGRESS').length, icon: PlayCircle, bg: 'bg-blue-600' },
                  { id: 'COMPLETED', l: 'Completed', c: productionFlow.filter(f => f.status === 'COMPLETED').length, icon: CheckCircle2, bg: 'bg-emerald-500' }
                ].map((s, i) => (
                  <button 
                    key={i} 
             ...
             <div className="grid grid-cols-1 xl:grid-cols-3 gap-2">
                {productionFlow
                  .filter(stage => flowFilter === 'ALL' || stage.status === flowFilter)
                  .map((stage, idx) => (
