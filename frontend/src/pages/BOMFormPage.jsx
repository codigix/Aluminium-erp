...
const BOMFormPage = () => {
  ...
  const itemId = useMemo(() => {
    ...
    return pathSegments.length > 1 ? pathSegments[pathSegments.length - 1] : null;
  }, [location.pathname]);
  ...
  const fetchData = useCallback(async (showLoading = true) => {
    try {
      ...
      const effectiveId = (itemId && itemId !== 'bom-form') ? itemId : (selectedItem?.id || null);

      if (effectiveId) {
        // If we have an ID but not selectedItem data (and it's not the one we just selected)
        if (!selectedItem || String(selectedItem.id) !== String(effectiveId)) {
          ...
          if (itemResponse.ok) {
            ...
            // Only auto-select if in read-only mode 
            // Otherwise we just fetch drawing info to filter, but let user select item manually
            if (isReadOnly) {
              setSelectedItem({ ...itemData, source: 'order' });
            }
            ...
            setProductForm(prev => ({
              ...
              drawing_id: itemData.drawing_id || prev.drawing_id,
        ...
        // Fetch BOM Details
        const itemCodeParam = selectedItem?.item_code || selectedItem?.itemCode;
        let bomUrl = `${API_BASE}/bom/items/${effectiveId}`;
        if (itemCodeParam) {
          bomUrl += `?itemCode=${encodeURIComponent(itemCodeParam)}`;
        }
  ...
  }, [itemId, selectedItem, isReadOnly]);
  ...
  // We only sync if there is NO itemId in the URL, or if the user manually selected something
  useEffect(() => {
    if (selectedItem) {
      // Logic to prevent auto-fill when coming from /bom-form/:id
      // but ALLOWING it if the user manually picked an item or it's a new form
      const isInitialLoadWithId = itemId && itemId !== 'bom-form' && !productForm.itemCode;

      if (!isInitialLoadWithId) {
        setProductForm(prev => ({
          ...prev,
        ...
        // Just fetch drawing info but don't fill product name/code/group
        setProductForm(prev => ({
          ...prev,
          drawingNo: selectedItem.drawing_no || prev.drawingNo,
          drawing_id: selectedItem.drawing_id || prev.drawing_id,
  ...
  const handleAddSectionItem = async (section, formData, setFormState, initialForm) => {
    try {
      const effectiveItemId = (itemId && itemId !== 'bom-form') ? itemId : (selectedItem?.id || null);
  ...
  const handleCreateBOM = async () => {
    try {
      ...
      const token = localStorage.getItem('authToken');
      const bomPayload = {
        itemId: selectedItem.source === 'order' ? selectedItem.id : null,
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-slate-900">
            ...
            <div>
              <h1 className="text-xl  flex items-center gap-3">
                {isReadOnly ? `Viewing BOM: ${itemId}` : 'Create BOM'}
            ...
            <div className="">
              {!isReadOnly && (
                <div className="bg-slate-50/50 p-3 rounded-lg border border-slate-100 mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px]  text-blue-600  tracking-wider">Quick Filter by Drawing</label>
                    <SearchableSelect
                      ...
                      onChange={(e) => {
                        ...
                        // Clear selection when drawing filter changes so user can pick manually from filtered list
                        if (newDwg) {
                          setSelectedItem(null);
                          setProductForm(prev => ({
                            ...prev,
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    disabled={isReadOnly}
                    className="w-full p-2 bg-white border border-slate-200 rounded-md text-xs font-medium text-slate-600 focus:ring-2 focus:ring-blue-500 focus:outline-none transition-all disabled:bg-slate-50 disabled:text-slate-400"
                ...
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    className={`w-full p-2 border border-slate-200 rounded-md text-xs font-medium transition-all focus:ring-2 focus:ring-blue-500 focus:outline-none ${(['Sub Assembly', 'Assembly'].includes(productForm.itemGroup) || isReadOnly) ? 'bg-slate-50 text-slate-400 cursor-not-allowed' : 'bg-white text-slate-600'}`}
                ...
                {['FG', 'SFG', 'Sub Assembly', 'Assembly'].includes(productForm.itemGroup) && productForm.drawingNo && (
                  <div className="space-y-1">
                    ...
                    <input type="text" disabled className="w-full p-2 bg-slate-50 border border-slate-200 rounded-md text-xs  text-blue-600" value={productForm.drawingNo} />
                  </div>
                )}
              </div>
              <div className="space-y-1">
        ...
        <Card className="p-0 border-slate-200 overflow-hidden shadow-sm">
          <div
            className="bg-white p-2 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors"
            ...
            <div className="flex items-center gap-2">
              {!isReadOnly && (
                <button
                  ...
                  className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs  hover:bg-indigo-700 transition-colors flex items-center gap-1 whitespace-nowrap"
                >
          ...
          {!collapsedSections.components && (
            <div className="p-2">
              {!isReadOnly && (
                <div className="bg-blue-50/30 p-2 rounded-md border border-orange-100 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group bg-white px-2 py-1 rounded border border-blue-100 shadow-sm">
                      ...
                      />
                      <span className="text-[10px]  text-slate-600 group-hover:text-blue-600 transition-colors">Show All Drawings</span>
                    </label>
                  </div>
                      ...
                      <SearchableSelect
                        ...
                        onChange={(e) => {
                          ...
                          setComponentForm({
                            ...
                            rate: item ? (item.selling_rate > 0 ? item.selling_rate : (item.valuation_rate || 0)) : componentForm.rate,
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-indigo-600 text-white rounded-lg text-xs  hover:bg-indigo-700 shadow-lg shadow-indigo-100 flex items-center justify-center min-w-[40px]"
                          title="Add Component"
                        >
                          +
                        </button>
                ...
                <div className="mt-4 overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
                  <table className="min-w-full divide-y divide-slate-200 bg-white">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Component</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Qty / UOM</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Rate</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Loss %</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Net Amount</th>
                        {!isReadOnly && <th className="px-4 py-3 text-right text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Actions</th>}
                      </tr>
                    </thead>
                        ...
                        return (
                          <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex flex-col">
                                <span className="text-sm font-semibold text-slate-800">{c.component_code}</span>
                            ...
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                              <span className="text-sm font-bold text-slate-900">{parseFloat(c.quantity).toFixed(2)}</span>
                              <span className="ml-1 text-[10px] font-medium text-slate-400">{c.uom}</span>
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600">
                              ₹{parseFloat(c.rate).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${parseFloat(c.loss_percent) > 0 ? 'bg-orange-50 text-orange-600 border border-orange-100' : 'text-slate-300'}`}>
                                {parseFloat(c.loss_percent || 0).toFixed(2)}%
                              </span>
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                              <span className="text-sm font-bold text-blue-600">
                                ₹{netCost.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </span>
                            </td>
                            {!isReadOnly && (
                              <td className="px-4 py-3 text-right whitespace-nowrap">
                                <button 
                                  onClick={() => handleDeleteSectionItem('components', c.id, c.isLocal)}
            ...
            <div className="flex items-center gap-2">
              {!isReadOnly && (
                <button
                  ...
                  className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs  hover:bg-emerald-700 transition-colors flex items-center gap-1 whitespace-nowrap"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                  </svg>
            ...
            <div className="p-2">
              {!isReadOnly && (
                <div className="bg-emerald-50/30 p-4 rounded-md border border-emerald-100 mb-4">
                  <div className="flex justify-between items-center mb-3">
                    <p className="text-[10px]  text-emerald-600">+ Add Raw Material</p>
                    <label className="flex items-center gap-2 cursor-pointer group bg-white px-2 py-1 rounded border border-emerald-100 shadow-sm">
                      <input
                        ...
                        onChange={(e) => setShowAllDrawings(e.target.checked)}
                      />
                      <span className="text-[10px]  text-slate-600 group-hover:text-emerald-600 transition-colors">Show All Drawings</span>
                    </label>
                  </div>
                      ...
                      <SearchableSelect
                        ...
                        onChange={(e) => {
                          ...
                          setMaterialForm({
                            ...
                            rate: item ? (item.selling_rate > 0 ? item.selling_rate : (item.valuation_rate || 0)) : materialForm.rate,
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-emerald-600 text-white rounded-lg text-xs  hover:bg-emerald-700 shadow-lg shadow-emerald-100 flex items-center justify-center min-w-[40px]"
                          title="Add Material"
                        >
                          +
                        </button>
              ...
              {bomData.materials.length > 0 && (
                <div className="mt-4 overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
                  <table className="min-w-full divide-y divide-slate-200 bg-white">
                    <thead className="bg-slate-50">
                      <tr>
                        <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Item Details</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Qty / UOM</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Rate</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Warehouse</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Operation</th>
                        <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Total Amount</th>
                        {!isReadOnly && <th className="px-4 py-3 text-right text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Actions</th>}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {bomData.materials.map((m) => {
                        ...
                        return (
                          <tr key={m.id} className="hover:bg-slate-50/80 transition-colors">
                            <td className="px-4 py-3 whitespace-nowrap">
                              <div className="flex flex-col">
                                <span className="text-sm font-semibold text-slate-800">{m.material_name || 'N/A'}</span>
                                <span className="text-[10px] text-slate-500 uppercase tracking-tight">{m.item_group || 'Raw Material'}</span>
                                {stockItem?.drawing_no && stockItem.drawing_no !== 'N/A' && (
                                  <span className="inline-flex items-center mt-1 text-[9px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-md border border-blue-100 w-fit">
                                    📐 Drg: {stockItem.drawing_no}
                                  </span>
                            ...
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                              <span className="text-sm font-bold text-slate-900">{qtyPerPc.toFixed(4)}</span>
                              <span className="ml-1 text-[10px] font-medium text-slate-400">{m.uom || 'Kg'}</span>
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600">
                              ₹{rate.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                              <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${m.warehouse ? 'bg-slate-100 text-slate-600' : 'text-slate-300'}`}>
                                {m.warehouse || 'Not Set'}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                               <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${m.operation ? 'bg-purple-50 text-purple-600 border border-purple-100' : 'text-slate-300'}`}>
                                {m.operation || 'None'}
                              </span>
                            </td>
                            <td className="px-4 py-3 text-center whitespace-nowrap">
                            ...
                            {!isReadOnly && (
                              <td className="px-4 py-3 text-right whitespace-nowrap">
                                <button 
                                  onClick={() => handleDeleteSectionItem('materials', m.id, m.isLocal)}
                                  className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all"
                                  title="Remove Item"
                                >
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center gap-2">
              {!isReadOnly && (
                <button
                  ...
                  className="px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs  hover:bg-purple-700 transition-colors flex items-center gap-1 whitespace-nowrap"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                  </svg>
            ...
            <div className="p-2">
              {!isReadOnly && (
                <div className="bg-purple-50/30 p-4 rounded-md border border-purple-100 mb-4">
                  <div className="mb-4">
                    <p className="text-xs  text-purple-700">This defines how the part is made</p>
                  ...
                  <div className="bg-white/50 p-3 rounded border border-purple-100 mb-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-[10px]  text-slate-500  mb-2">Cost Formula:</p>
                      <code className="text-[10px] bg-slate-100 p-2 rounded block text-slate-700">
                        Operation Cost/Unit = (Cycle + (Setup / Batch Qty)) / 60 × Hourly Rate
                      </code>
                    </div>
                    <div>
                      <p className="text-[10px]  text-slate-500  mb-2">🔒 System Rule</p>
                      <p className="text-[10px] text-slate-600 leading-relaxed">
                        Operations auto-create Work Orders. Target warehouse controls material movement
                        <span className=" text-purple-600 mx-1 text-[9px]">RM → WIP → FG</span>
                      </p>
                    </div>
                  </div>

                  <p className="text-[10px]  text-purple-600  mb-3">+ Add Operation</p>
                  <div className="grid grid-cols-1 md:grid-cols-6 gap-3 mb-3">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="text" readOnly className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs  text-purple-600" value={(((parseFloat(operationForm.cycleTimeMin || 0) + parseFloat(operationForm.setupTimeMin || 0)) / 60) * parseFloat(operationForm.hourlyRate || 0)).toFixed(2)} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="flex flex-col justify-end">
                      <button onClick={() => handleAddSectionItem('operations', operationForm, setOperationForm, { operationName: '', workstation: '', cycleTimeMin: '', setupTimeMin: '', hourlyRate: '', operationType: 'In-House', targetWarehouse: '' })} className="p-2 bg-purple-600 text-white rounded-lg text-xs  hover:bg-purple-700 shadow-lg shadow-purple-100">+ Add</button>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  {bomData.operations.length > 0 && (
                    <div className="mt-4 overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
                      <table className="min-w-full divide-y divide-slate-200 bg-white">
                        <thead className="bg-slate-50">
                          <tr>
                            <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Operation / Workstation</th>
                            <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Std Cycle</th>
                            <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Setup Ref</th>
                            <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Rate/hr</th>
                            <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Total Time</th>
                            <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Op. Cost</th>
                            {!isReadOnly && <th className="px-4 py-3 text-right text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Actions</th>}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {bomData.operations.map((o) => {
                            ...
                            return (
                              <tr key={o.id} className="hover:bg-slate-50/80 transition-colors">
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="flex flex-col">
                                    <span className="text-sm font-semibold text-slate-800">{o.operation_name}</span>
                                    <span className="text-[10px] text-slate-500 font-medium bg-slate-100 px-1.5 py-0.5 rounded w-fit mt-1">
                                      🏢 {o.workstation || 'N/A'}
                                    </span>
                                  </div>
                                </td>
                                <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600 font-medium">{cycleTime}m</td>
                                <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600 font-medium">{setupTime}m</td>
                                <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600">
                                  ₹{hourlyRate.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </td>
                                <td className="px-4 py-3 text-center whitespace-nowrap">
                                  <span className="text-sm font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100">
  ...
  {/* SECTION 5: Scrap & Loss */}
          <Card className="p-0 border-slate-200 overflow-hidden shadow-sm">
              ...
              <div className="flex items-center gap-2">
                {!isReadOnly && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-orange-600 text-white rounded-lg text-xs  hover:bg-orange-700 transition-colors flex items-center gap-1 whitespace-nowrap"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                    </svg>
            ...
            {!collapsedSections.scrap && (
              <div className="p-2">
                {!isReadOnly && (
                  <div className="bg-orange-50/30 p-2 rounded-md border border-orange-100 mb-4">
                    <div className="flex justify-between items-center mb-3">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group bg-white px-2 py-1 rounded border border-orange-100 shadow-sm">
                        <input
                          ...
                          onChange={(e) => setShowAllDrawings(e.target.checked)}
                        />
                        <span className="text-[10px]  text-slate-600 group-hover:text-orange-600 transition-colors">Show All Drawings</span>
                      </label>
                    </div>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="p-2 bg-orange-600 text-white rounded-lg text-xs  hover:bg-orange-700 shadow-lg shadow-orange-100 flex items-center justify-center min-w-[40px]"
                            title="Add Scrap"
                          >
                            +
                          </button>
                    ...
                    {bomData.scrap.length > 0 && (
                      <div className="mt-4 overflow-x-auto border border-slate-200 rounded-xl shadow-sm">
                        <table className="min-w-full divide-y divide-slate-200 bg-white">
                          <thead className="bg-slate-50">
                            <tr>
                              <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Item / Code</th>
                              <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Input Qty</th>
                              <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Loss %</th>
                              <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Scrap Qty</th>
                              <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Rate</th>
                              <th className="px-4 py-3 text-center text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Scrap Value</th>
                              {!isReadOnly && <th className="px-4 py-3 text-right text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Actions</th>}
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100">
                            {bomData.scrap.map((s) => {
                              ...
                              return (
                                <tr key={s.id} className="hover:bg-slate-50/80 transition-colors">
                                  <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="flex flex-col">
                                      <span className="text-sm font-semibold text-slate-800">{s.item_name || 'N/A'}</span>
                                      <span className="text-[10px] text-slate-500 uppercase tracking-tight">{s.item_code || 'N/A'}</span>
                                      {stockItem?.drawing_no && stockItem.drawing_no !== 'N/A' && (
                                        <span className="inline-flex items-center mt-1 text-[9px] font-medium text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-md border border-blue-100 w-fit">
                                          📐 Drg: {stockItem.drawing_no}
                                        </span>
                                  ...
                                  <td className="px-4 py-3 text-center whitespace-nowrap text-sm font-bold text-slate-900">{scrapQty.toFixed(2)}</td>
                                  <td className="px-4 py-3 text-center whitespace-nowrap text-sm text-slate-600">
                                    ₹{rate.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                  </td>
                                  <td className="px-4 py-3 text-center whitespace-nowrap">
                  ...
                  <div className="bg-white border border-slate-100 rounded-md overflow-hidden">
                    <div className="divide-y divide-slate-50">
                      ...
                      <div className="px-4 py-3 flex justify-between items-center bg-blue-50/50">
                        <span className="text-xs  text-blue-700 font-bold">Material Cost (after Scrap):</span>
                        <span className="text-xs  text-blue-900 font-bold">₹{materialCostAfterScrap.toFixed(2)}</span>
                      </div>
                      <div className="px-4 py-3 flex justify-between items-center hover:bg-slate-50 transition-colors text-purple-600">
                        ...
                        <span className="text-xs  text-purple-900 font-bold">₹{operationsCost.toFixed(2)}</span>
                      </div>
